...
- Cálculo de média de produção por hora (RF-T04).

Observação: MachineStop/StopReason virá na Sprint 4 (mesma app).
'''
...
from catalogs.models import GridModel, Machine, Shift, Polarity
...
class GridProduction(TimestampedModel):
    ...
    @property
    def stop_hours(self):
        '''Soma de horas paradas vinculadas (Sprint 4 preencherá MachineStop).'''
        stops = getattr(self, 'stops', None)
        total = 0
        if stops is None:
            return total
        for stop in stops.all():
            if stop.stop_start and stop.stop_end and stop.stop_end > stop.stop_start:
                total += (stop.stop_end - stop.stop_start).total_seconds() / 3600.0
        return total
    ...
    def production_per_hour(self):
        '''Média de produção por hora (líquido de paradas) — RF-T04.'''
